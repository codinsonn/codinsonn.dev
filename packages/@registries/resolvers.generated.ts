// -i- Auto generated with "yarn collect-resolvers" -- /packages/@aetherspace/scripts/collect-resolvers.ts
export { graphResolver as healthCheck } from 'app/routes/api/health/route'
export { graphResolver as getResumeDataByUserSlug } from 'cv-page/routes/api/resume/[slug]/route'
export { graphResolver as upsertResume } from 'cv-page/routes/api/resume/[slug]/upsert/route'
export { graphResolver as getUserBio } from 'links-page/routes/api/bio/[slug]/route'
export { graphResolver as syncUserBiosFromAirtable } from 'links-page/routes/api/links/syncs/airtable/route'
export { graphResolver as getShopifyProducts } from '@aetherspace/commerce/routes/api/aetherspace/commerce/shopify/products/route'
